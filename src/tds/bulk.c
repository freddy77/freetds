...
tds7_send_record(TDSSOCKET *tds, TDSBCPINFO *bcpinfo,
		 tds_bcp_get_col_data get_col_data, tds_bcp_null_error null_error, int offset, int start_col)
{
	int i;
	...
	for (i = start_col; i < bcpinfo->bindinfo->num_cols; i++) {
		...
		if ((!bcpinfo->identity_insert_on && bindcol->column_identity) || bindcol->column_timestamp
		    || (bindcol->column_computed && !bcpinfo->with_triggers) || !tds_bcp_is_bound(bcpinfo, bindcol)) {
			continue;
		}
		    ...
		    tds_bcp_get_col_data get_col_data, tds_bcp_null_error null_error, int offset)
{
	...
	if (start_col > 0) {
		...
		*bindcol->column_lenbind
			= TDS_MIN((TDS_INT) bindcol->column_bindlen
			      - bcpinfo->text_sent,
			      *bindcol->column_lenbind);
		if (IS_TDS7_PLUS(tds->conn)
	...
	if (IS_TDS7_PLUS(tds->conn))
		rc = tds7_send_record(tds, bcpinfo, get_col_data, null_error, offset, start_col);
	else
		rc = tds5_send_record(tds, bcpinfo, get_col_data, null_error, offset, start_col);
...
tds7_bcp_send_colmetadata(TDSSOCKET *tds, TDSBCPINFO *bcpinfo)
{
	...
	for (i = 0; i < bcpinfo->bindinfo->num_cols; i++) {
		...
		if ((!bcpinfo->identity_insert_on && bcpcol->column_identity) || bcpcol->column_timestamp
		    || (bcpcol->column_computed && !bcpinfo->with_triggers) || !tds_bcp_is_bound(bcpinfo, bcpcol)) {
			continue;
		}
		num_cols++;
	}
